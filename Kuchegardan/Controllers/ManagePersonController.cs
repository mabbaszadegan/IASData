...
namespace Kuchegardan.Controllers
{
    ...
    public class ManagePersonController : Controller
    {
        ...
        public ActionResult PersonIndex(int? tabId = null, int pageId = 0)
        {
            ...
            //List<FamilyViewModel> familyList = new List<FamilyViewModel>();
            //try
            //{
            //    if (User.Identity.IsAuthenticated)
            //    {
            //        var user = db.UserInfoRepository.Get(u => u.UserName == User.Identity.Name).SingleOrDefault();
            //        List<DAL.DepartmentViewModel> departments = user.UserDepartment.Where(q => q.Department.Person.Any() && q.Department.DepartmentIsActive).Select(q => new DepartmentViewModel
            //        {
            //            DepartmentId = q.DepartmentId,
            //            DepartmentName = q.Department.DepartmentName,
            //            DepartmentCode = q.Department.DepartmentCode,
            //            PersonsCount = q.Department.Person.Count(),
            //            FamiliesCount = db.FamilyMemberRepository.Get(p => p.Person.DepartmentId == q.DepartmentId).Select(p => p.FamilyId).Distinct().Count()
            //        }
            //         ).ToList();
            ...
            //    }
            //    var families = db.FamilyRepository.Get(q => q.FamilyMember.FirstOrDefault().Person.DepartmentId == tabId);

            //    int pageCount = families.Count() / pageSize;
            //    if (families.Count() % pageSize > 0)
            //        pageCount++;
            //    ViewBag.PageCount = pageCount;
            //    foreach (var item in families.OrderBy(q => q.FamilyCode).Skip(skip).Take(pageSize))
            //    {
            //        try
            //        {
            //            familyList.Add(new FamilyViewModel
            //            {
            //                FamilyId = item.FamilyId,
            //                FamilyCode = item.FamilyCode,
            //                DepartmentId = item.FamilyMember.FirstOrDefault().Person.DepartmentId,
            //                SupervisorName = (
            //                                item.FamilyMember.Any(q => q.FamilyMemberIsParent) ?
            //                                item.FamilyMember.Select(q => new { name = q.Person.PersonFirstName + " " + q.Person.PersonLastName + "(" + q.RelationType.RelationTypeName + ")", q.FamilyMemberIsParent }).FirstOrDefault(q => q.FamilyMemberIsParent).name :
            //                                item.FamilyMember.Select(q => new { name = q.Person.PersonFirstName + " " + q.Person.PersonLastName + "(" + q.RelationType.RelationTypeName + ")" }).FirstOrDefault().name
            //                                ),
            //                FamilyAddress = item.FamilyAddress,
            //                FamilyDesc = item.FamilyDesc,
            //                MembersCount = item.FamilyMember.Count(),
            //                Phone = item.FamilyPhone1,
            //            });
            //        }
            //        catch (Exception ex) { }
            //    }
            ...
            //    var input = "tabid=" + tabId + "&pageid=" + pageId;
            //    var output = new JavaScriptSerializer().Serialize(familyList);
        ...
        public ActionResult CreatePerson(PersonViewModel personViewModel)
        {
            ...
            else person.PersonBirthDay = null;
            ...
            else person.PersonBirthMonth = null;
            ...
            else person.PersonBirthYear = null;
        ...
        public ActionResult Search(DAL.FamilySearchViewModel model, int? tabId = null, int pageId = 0)
        {
            int pageSize = 20;
            int skip = pageId * pageSize;
            List<FamilyViewModel> familyList = new List<FamilyViewModel>();
            try
            {
                ...
                if (User.Identity.IsAuthenticated)
                {
                    ...
                    if (model.DepartmentId > 0)
                        families = families.Where(q => q.FamilyMember.Any(p => p.Person.DepartmentId == model.DepartmentId));
                    if (model.NationalityId > 0)
                        families = families.Where(q => q.FamilyMember.Any(p => p.Person.NationalityId == model.NationalityId));
                    if (model.EthnicId > 0)
                        families = families.Where(q => q.FamilyMember.Any(p => p.Person.EthnicId == model.EthnicId));
                    if (model.PersonStatusId > 0)
                        families = families.Where(q => q.FamilyMember.Any(p => p.Person.PersonStatusId == model.PersonStatusId));
                    if (model.RelationTypeId > 0)
                        families = families.Where(q => q.FamilyMember.Any(p => p.RelationTypeId == model.RelationTypeId));
                    if (model.GenderId > 0)
                        families = families.Where(q => q.FamilyMember.Any(p => p.Person.GenderId == model.GenderId));
                    if (model.MaritalStatusId > 0)
                        families = families.Where(q => q.FamilyMember.Any(p => p.Person.MaritalStatusId == model.MaritalStatusId));

                    if (!string.IsNullOrEmpty(model.PersonFirstName))
                        families = families.Where(q => q.FamilyMember.Any(p => p.Person.PersonFirstName.Contains(model.PersonFirstName)));
                    if (!string.IsNullOrEmpty(model.PersonLastName))
                        families = families.Where(q => q.FamilyMember.Any(p => p.Person.PersonLastName.Contains(model.PersonLastName)));
                    if (!string.IsNullOrEmpty(model.PersonFatherName))
                        families = families.Where(q => q.FamilyMember.Any(p => p.Person.PersonFatherName.Contains(model.PersonFatherName)));
                    if (!string.IsNullOrEmpty(model.PersonMotherName))
                        families = families.Where(q => q.FamilyMember.Any(p => p.Person.PersonMotherName.Contains(model.PersonMotherName)));
                    if (!string.IsNullOrEmpty(model.PersonNationalCode))
                        families = families.Where(q => q.FamilyMember.Any(p => p.Person.PersonNationalCode.Contains(model.PersonNationalCode)));
                    if (!string.IsNullOrEmpty(model.PersonBirthYearFrom))
                        families = families.Where(q => q.FamilyMember.Any(p => p.Person.PersonBirthYear.CompareTo(model.PersonBirthYearFrom) >= 0));
                    if (!string.IsNullOrEmpty(model.PersonBirthYearTo))
                        families = families.Where(q => q.FamilyMember.Any(p => p.Person.PersonBirthYear.CompareTo(model.PersonBirthYearTo) <= 0));

                    List<DAL.DepartmentViewModel> departments = new List<DepartmentViewModel>();
                    departments = families.Select(q => new DepartmentViewModel
                    {
                        ...
                        //FamiliesCount = db.FamilyMemberRepository.Get(p => p.Person.DepartmentId == item.FamilyMember.FirstOrDefault().Person.DepartmentId).Select(p => p.FamilyId).Distinct().Count()

                    }).Distinct().ToList();
                    ViewBag.Departments = departments;
                    if (!tabId.HasValue)
                        tabId = departments.OrderBy(q => q.DepartmentCode).First().DepartmentId;
                    families = families.Where(q => q.FamilyMember.FirstOrDefault().Person.DepartmentId == tabId);
        ...
        private void InitForm(PersonViewModel person)
        {
            ...
            beliveId.Add(new Belive { BeliveName = "" });
            ...
            EthnicId.Add(new Ethnic { EthnicName = "" });
            ...
            maritalStatusId.Add(new MaritalStatus { MaritalStatusName = "" });
            ...
            nationalityId.Add(new Nationality { NationalityName = "" });
            ...
            personStatusId.Add(new PersonStatus { PersonStatusName = "" });
            ...
            religionId.Add(new Religion { ReligionName = "" });
            ...
            relationTypeId.Add(new RelationType { RelationTypeName = "" });
        ...
        private void InitForm()
        {
            ...
            List<DAL.Department> departments = user.UserDepartment.Where(q => q.Department.DepartmentIsActive).Select(q => new Department
            {
                DepartmentId = q.DepartmentId,
                DepartmentName = q.Department.DepartmentName,
                DepartmentCode = q.Department.DepartmentCode,
            ...
            genders.Add(emtyItem);
            genders.AddRange(db.GenderRepository.Get().Select(q => new LookupModel { Value = q.GenderId.ToString(), Name = q.GenderName }).OrderBy(q => q.Name));
            ViewBag.GenderId = new SelectList(genders, "Value", "Name");
            ...
            List<LookupModel> personStatusId = new List<LookupModel>();
            personStatusId.Add(new LookupModel { Value = "0", Name = "" });
            personStatusId.AddRange(db.PersonStatusRepository.Get().Select(q => new LookupModel { Value = q.PersonStatusId.ToString(), Name = q.PersonStatusName }).OrderBy(q => q.Name));
            ViewBag.PersonStatusId = new SelectList(personStatusId, "Value", "Name");
